// Pass 52ie: refreshed 1-150B size>=20B ranks 1-5 (post >150B exhaustion check)
// @category Analysis
...
public class ColdTriageRegion80040000Pass52ie2 extends GhidraScript {
    public void run() throws Exception {
        ...
        while (fiter.hasNext()) {
            ...
            if (size <= 150 && size >= 20) allSmall.add(new long[]{addr, xrefs, size});
        }
        allSmall.sort((a, b) -> {
            if (b[1] != a[1]) return Long.compare(b[1], a[1]);
            return Long.compare(b[2], a[2]);
        });
        println("Total: " + total + " Unnamed: " + unnamed + " 1-150B size>=20B: " + allSmall.size());
        for (int i = 0; i < Math.min(5, allSmall.size()); i++) {
