...
## Pass 54ao (2026-06-28) — cold-triage rank-1 rename (post-54an re-rank; xrefs=0 tier)

Fresh `ColdTriageRegion80050000Pass54.java` re-run after Pass 54an: **366 total**, **352 named**,
**14 unnamed** (9 artifacts excluded). All remaining candidates are **xrefs=0**. Decompiled and
renamed rank-1 **`FUN_8005bf4c` → `dispatch_link_power_mode_by_status_bits_and_commit`**
(252B, HIGH) via `RenamePass54aoFun8005bf4c.java` (`renamed=1`, live-verified).

**Mechanism:** Link power-mode transition dispatcher (promoted from Pass 14/15 MEDIUM-HIGH
triage): gates on `validate_connection_setup_preconditions` and an active-link bit check, then
branches on the 2-bit low-power encoding at `+0x106 & 0x6000` (`0x2000`/`0x4000`/`0x6000` —
same bit family `commit_link_power_state_bits_to_hw_register_with_retry` writes to HW). Mode
`0x2000` ORs flag byte `8` into `+0x104`; mode `0x6000` increments a counter at `+0x109` until
threshold `+0x108`, then ORs flag `4`; mode `0x4000` (when `field407_0x1a4 & 1`) calls
`param_dispatch_with_rom_calls` with a clock-derived timing delta. Common tail dispatches via
`FUN_80033794`→`FUN_80035768` or sets `+0x8c` and calls `FUN_80034ccc` depending on a global
flag bit.

**Confidence:** HIGH — unambiguous 2-bit mode dispatch on established `0x6000` encoding +
named `validate_connection_setup_preconditions` / `param_dispatch_with_rom_calls` callees;
independently corroborates Pass 15's `commit_link_power_state_bits_to_hw_register_with_retry`
semantic assignment.

Region unnamed count after this pass: **14** (15 minus this rename).
