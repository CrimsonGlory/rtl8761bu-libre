...
## Pass 12ed (2026-06-29) — PSM/QoS eligibility bitmask builder `FUN_80065068`

Decompiled and renamed:
**`FUN_80065068` → `build_psm_qos_channel_eligibility_bitmask_0x50`**
(456B, HIGH, COMPLEX-tier) via `RenamePass12edRegion80070000Fun80065068.java` (`renamed=1`, live-verified).

**Mechanism:** Optional early-exit hook at `PTR_DAT_80065230`. `memset` clears 10-byte output at `param_1`, then scans ACL channel indices `0..0x4e` step 2: consults enable masks `PTR_DAT_80065234`/`8006523c`, per-channel state `PTR_DAT_80065238`, 0xe-stride struct `PTR_DAT_80065240`, timing shorts `PTR_DAT_80065248` vs config `field296_0x134`/`field297_0x135`, and candidate lists `PTR_DAT_80065250`/`80065254`/`8006525c`/`80065260`; packs 2-bit channel flags (or `0x40` on byte 9 for index `0x4e`). Returns populated-bit count. First step in PSM/QoS dispatch (`FUN_800661b8`, `FUN_80066330`, `FUN_800666a0` ← `FUN_80066914`). Lives at `0x80065068` (region `0x80060000`).

**Confidence:** HIGH — full decompilation; upstream sibling of `run_psm_qos_dual_quantizer_search_and_emit_lmp_0x25c` in the quantizer cluster.
...
**Mechanism:** Takes channel-slot index `param_1`. `check_psm_qos_channel_slot_eligibility_scan10` validates per-slot enable flag bit 1 and scans up to 10 eligibility entries via `test_psm_qos_channel_slot_eligibility_entry`; on success `aggregate_acl_channel_timing_averages_into_10byte_buffer` aggregates ACL timing stats over 0x4f channels into a 10-byte buffer and calls `fill_psm_qos_10byte_channel_timing_entries_from_acl_with_qos_adjust`. Stores current slot to `PTR_DAT_80064fac`; when global mode dword at `PTR_DAT_80064fb0` is `-1`, emits VSC 0xFC95 via `VSC_0xfc95_called2`; always finishes with `LMP__268__most_common_for_VSCs2_checks_fptr_patch`. Returns 1 on success, 0 if slot ineligible. Callee of `gate_psm_qos_dual_quantizer_or_iter_channel_slots` on the non-dual-quantizer channel-slot iteration path. Lives at `0x80064f60` (region `0x80060000`).
...
**Mechanism:** Takes channel-slot index `param_1`. When per-slot enable flag bit 1 is set at `PTR_DAT_80063f6c[(slot)+4]`, scans eligibility entries `0..9` via `test_psm_qos_channel_slot_eligibility_entry(entry, slot)`; returns 1 if any entry passes, else 0. Sole eligibility gate before `validate_psm_qos_channel_slot_and_emit_lmp_0x268` proceeds to timing setup + LMP 0x268 emit. Lives at `0x80063f28` (region `0x80060000`).
...
## Pass 12ew (2026-06-29) — PSM/QoS per-entry channel-slot eligibility test `FUN_80063ea0`

Decompiled and renamed:
**`FUN_80063ea0` → `test_psm_qos_channel_slot_eligibility_entry`**
(124B, HIGH, SIMPLE-tier) via `RenamePass12ewRegion80070000Fun80063ea0.java` (`renamed=1`, live-verified).

**Mechanism:** Takes entry index `param_1` and channel-slot index `param_2`. Optional early-exit hook at `PTR_PTR_80063f1c`; default path requires per-slot enable bit 1 at `PTR_DAT_80063f20[(slot)+4]`, non-zero status at `bos[bosi]._xb2_byte_minus_4_used_as_status_array_index`, and slot match against `bos[bosi].byte_0xCC`. Returns 1 when `field_0x219==1` or `field_0x243==1`, else 0. Sole per-entry test invoked by `check_psm_qos_channel_slot_eligibility_scan10` entries `0..9`. Lives at `0x80063ea0` (region `0x80060000`).

**Confidence:** HIGH — full decompilation; per-entry leaf in PSM/QoS channel-slot eligibility cluster.
...
Decompiled and renamed:
**`FUN_8007882c` → `noop_unused_patch_hook_slot_vsc_fc13_stub_gap_head_return_ff`**
(4B, HIGH, STUB-tier) via `RenamePass12gyRegion80070000Fun8007882c.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 STUB-tier candidate per `ListStub80070000.java` listing
(4B, xref_in=0 — MCP `xrefs_to` returns empty against this GZF, known gap).

**Mechanism:** Trivial `return 0xff;` unused patch-hook slot stub occupying the head
slot of the four-byte gap cluster at `0x8007882c`–`0x80078834` in the `0x800788xx`
patch-hook cluster — sits immediately after `VSC_0xfc13_return_one_default_stub`
at `0x80078828` (which returns `1`) and before Pass 12gx's
`noop_unused_patch_hook_slot_vsc_fc13_stub_gap_return_zero` at `0x80078830`.
No callers; likely an unused patch-hook slot placeholder that patch firmware may
repurpose (the `0xff` sentinel contrasts with the live VSC 0xFC13 stub's `return 1;`
and sibling gap stubs' `return 0;` / void noop).

**Confidence:** HIGH — unambiguous constant return; cluster placement confirmed
...
Decompiled and renamed:
**`FUN_80078830` → `noop_unused_patch_hook_slot_vsc_fc13_stub_gap_return_zero`**
(4B, HIGH, STUB-tier) via `RenamePass12gxRegion80070000Fun80078830.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 STUB-tier candidate per `ListStub80070000.java` listing
(4B, xref_in=0 — MCP `xrefs_to` returns empty against this GZF, known gap).

**Mechanism:** Trivial `return 0;` unused patch-hook slot stub occupying the middle
slot of the four-byte gap cluster at `0x8007882c`–`0x80078834` in the `0x800788xx`
patch-hook cluster — sits immediately after `VSC_0xfc13_return_one_default_stub`
at `0x80078828` and before Pass 12gw's
`noop_unused_patch_hook_slot_vsc_fc13_stub_gap_tail` at `0x80078834`. No
callers; likely an unused patch-hook slot placeholder that patch firmware may
repurpose (contrasts with the live VSC 0xFC13 stub's `return 1;`).

**Confidence:** HIGH — unambiguous constant return; cluster placement confirmed
by adjacent named siblings; zero xref_in consistent with dead/unused hook slot.

Live named **1359** (global; in-region unnamed **12**; STUB-tier unnamed **10**).

**Next:** superseded by Pass 12gy.
...
**Next:** superseded by Pass 12gx.

## Pass 12gv (2026-06-30) — patch-hook gap noop stub `FUN_80078838`

Decompiled and renamed:
**`FUN_80078838` → `noop_unused_patch_hook_slot_before_vsc_fc13_stub_gap`**
(4B, HIGH, STUB-tier) via `RenamePass12gvRegion80070000Fun80078838.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 STUB-tier candidate per `ListStub80070000.java` listing
(4B, xref_in=0 — MCP `xrefs_to` returns empty against this GZF, known gap).

**Mechanism:** Trivial `return;` noop stub occupying a 4-byte gap in the
`0x800788xx` patch-hook cluster — sits immediately after the four-byte gap
stubs at `0x8007882c`–`0x80078834` and immediately before Pass 12gu's
`noop_unused_patch_hook_slot_after_vsc_fc13_stub_gap` at `0x8007883c`. No callers;
likely an unused patch-hook slot placeholder that patch firmware may repurpose.
...
Decompiled and renamed:
**`FUN_8007883c` → `noop_unused_patch_hook_slot_after_vsc_fc13_stub_gap`**
(4B, HIGH, STUB-tier) via `RenamePass12guRegion80070000Fun8007883c.java` (`renamed=1`, live-verified).
...
**Mechanism:** Trivial `return;` noop stub occupying a 4-byte gap in the
`0x800788xx` patch-hook cluster — sits immediately after the four-byte gap
stubs at `0x8007882c`–`0x80078834` and immediately before Pass 12gt's
`noop_unused_patch_hook_slot_after_vsc_fc13_tail` at `0x80078840`. No callers;
likely an unused patch-hook slot placeholder that patch firmware may repurpose.

**Confidence:** HIGH — unambiguous trivial return; cluster placement confirmed
by adjacent named siblings; zero xref_in consistent with dead/unused hook slot.

Live named **1356** (global; in-region unnamed **15**; STUB-tier unnamed **13**).

**Next:** superseded by Pass 12gv.

## Pass 12gt (2026-06-30) — patch-hook gap noop stub `FUN_80078840`

Decompiled and renamed:
**`FUN_80078840` → `noop_unused_patch_hook_slot_after_vsc_fc13_tail`**
(4B, HIGH, STUB-tier) via `RenamePass12gtRegion80070000Fun80078840.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 STUB-tier candidate per `ListStub80070000.java` listing
(4B, xref_in=0 — MCP `xrefs_to` returns empty against this GZF, known gap).

**Mechanism:** Trivial `return;` noop stub occupying a 4-byte gap in the
`0x800788xx` patch-hook cluster — sits immediately after Pass 12gl's
`VSC_0xfc13_return_one_default_stub` at `0x80078828` and immediately before
Pass 12gs's `noop_unused_patch_hook_slot_before_role_switch_tail` at
`0x80078844`. No callers; likely an unused patch-hook slot placeholder that
patch firmware may repurpose.

**Confidence:** HIGH — unambiguous trivial return; cluster placement confirmed
by adjacent named siblings; zero xref_in consistent with dead/unused hook slot.

Live named **1355** (global; in-region unnamed **16**; STUB-tier unnamed **14**).

**Next:** superseded by Pass 12gu.

## Pass 12gs (2026-06-30) — patch-hook gap noop stub `FUN_80078844`

Decompiled and renamed:
**`FUN_80078844` → `noop_unused_patch_hook_slot_before_role_switch_tail`**
(4B, HIGH, STUB-tier) via `RenamePass12gsRegion80070000Fun80078844.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 STUB-tier candidate per `ListStub80070000.java` listing
(4B, xref_in=0 — MCP `xrefs_to` returns empty against this GZF, known gap).

**Mechanism:** Trivial `return;` noop stub occupying a 4-byte gap in the
`0x800788xx` patch-hook cluster — sits immediately after the VSC 0xFC13 stub
cluster tail at `0x8007882c` and immediately before Pass 12gk's
`noop_role_switch_completion_tail_hook` at `0x80078848`. No callers; likely an
unused patch-hook slot placeholder that patch firmware may repurpose.

**Confidence:** HIGH — unambiguous trivial return; cluster placement confirmed
by adjacent named siblings; zero xref_in consistent with dead/unused hook slot.

Live named **1354** (global; in-region unnamed **17**; STUB-tier unnamed **15**).

**Next:** superseded by Pass 12gt.
...
**Mechanism:** Takes channel-slot index `param_1`. Iterates ACL channel indices `0..0x4e` (79 channels): requires per-channel active flag at `PTR_DAT_80064f50[i]` and enable bit in `PTR_DAT_80064f54[i>>3]`. For each matching 14-byte ACL record at `PTR_DAT_80064f58 + i*0xe`, when `entry[0]!=0` and `entry[1]==0`, accumulates `entry[5]` and `entry[0]` into running sums with a match count. On zero matches, zeros the derived averages; else divides sums by count. Packs a 10-byte PSM/QoS template at `PTR_DAT_80064f5c` (averaged timing fields at bytes 0–1 and 7–9, fixed constants `0x1e/10/9` at bytes 4–6, zeros at 2–3), then calls `fill_psm_qos_10byte_channel_timing_entries_from_acl_with_qos_adjust(0, 10, param_1)`. Callee of `validate_psm_qos_channel_slot_and_emit_lmp_0x268` on the eligible channel-slot path. Lives at `0x80064e98` (region `0x80060000`).
...
Decompiled and renamed:
**`FUN_800643a0` → `build_psm_qos_5byte_eligibility_bitmask_from_10byte_template`**
(152B, HIGH, SIMPLE-tier) via `RenamePass12esRegion80070000Fun800643a0.java` (`renamed=1`, live-verified).

**Mechanism:** Optional early-exit hook at `PTR_DAT_80064438`. Default path: (1) walks 2-bit pairs across the 10-byte input `param_2`, decoding value `3` into paired eligibility flags in scratch `PTR_DAT_8006443c` (77 iterations, step 2); (2) special-case byte-9 bit6 → flag at offset `0x4e`; (3) iterates channel indices `0..0x24`, using lookup table `PTR_DAT_80064440`, and when scratch flag at `entry*2` is set, ORs the corresponding bit into output buffer `param_1` (5-byte eligibility bitmask fed to `dispatch_psm_qos_opcode_mode_merge_bitmask_and_sync`). Callee of Pass 12er.

**Confidence:** HIGH — full decompilation; bitmask-builder leaf in PSM/QoS opcode-mode cluster.
...
Live named **1301** (global; in-region unnamed **49**).
...
Live named **1298** (global; in-region unnamed **52**).
...
Decompiled and renamed:
**`FUN_800658f0` → `expand_psm_qos_state_0x16_even_retry_bitpair_adjust_eligibility`**
(894B, HIGH, HANDLER-tier) via `RenamePass12emRegion80070000Fun800658f0.java` (`renamed=1`, live-verified).

**Mechanism:** Sole caller `run_psm_qos_state_0x16_eligibility_subdispatch` when per-channel retry counter is even and `>=2` (`(counter & 1)==0` but `counter != 0`). Uses per-channel 6-byte state at `PTR_DAT_80065c70[param_1*6]`. When state byte `[4]==1`: bidirectional bit-pair walk on 10-byte output `param_3` guided by encoded channel index in `[0]` (high/low nibbles = byte index + bit offset), toggling direction flag `[5]`, comparing against reference buffer `PTR_DAT_80065c78`; sub-state byte on `PTR_DAT_80065c7c[param_1]` values `0x04`/`0x08` gate walk direction. When `[4]==0xff`: up to 10 random-index attempts via `FUN_80042934(0x4e)` (same PRNG helper as retry-1). Updates eligibility count `*param_2` and advances `PTR_DAT_80065c7c[param_1]` retry sub-state (+1 or +10 on threshold miss).

**Confidence:** HIGH — full decompilation; even-retry sibling of Pass 12el retry-0 sequential walk and Pass 12ek retry-1 random walk in Pass 12eg sub-state machine.

Live named **1296** (global; in-region unnamed **54**).

## Pass 12el (2026-06-29) — PSM/QoS state-0x16 retry-0 sequential bitpair walk `FUN_800640c4`

Decompiled and renamed:
**`FUN_800640c4` → `expand_psm_qos_state_0x16_retry0_sequential_channel_bitpair_eligibility`**
(316B, HIGH, MEDIUM-tier) via `RenamePass12elRegion80070000Fun800640c4.java` (`renamed=1`, live-verified).

**Mechanism:** Sole caller `run_psm_qos_state_0x16_eligibility_subdispatch` when per-channel retry counter `PTR_DAT_80066308[param_1]==0`. When `param_2 < 0x46`: sequentially walks channel indices `0..0x4e`, gating on occupancy byte `PTR_DAT_80064208[uVar7]` and eligibility bitmask `PTR_DAT_80064210[uVar7>>3]` bit `(uVar7&7)`. Builds paired channel-index assignments in dual byte buffers (`local_10`/`local_c`), comparing span against config threshold `field295_0x133`. Special case at index `0x4e` when bitmask bit `0x40` clear and alternating-toggle state set. Stores eligibility count in `PTR_DAT_80064214` and per-channel success flag + retry increment in `PTR_DAT_80064218`/`8006421c`.

**Confidence:** HIGH — full decompilation; retry-0 sibling of Pass 12ek random walk and even-retry `expand_psm_qos_state_0x16_even_retry_bitpair_adjust_eligibility` in Pass 12eg sub-state machine; uses same config threshold as cluster.

Live named **1295** (global; in-region unnamed **55**).
...
**Mechanism:** Unpacks a bit-packed 6-byte field from an LMP PDU buffer starting at `param_1+4`: merges bytes at offsets `+4`…`+10` with shifts `(byte+5)<<6`, `(byte+6)<<0xe`, and `(byte+7)<<0x16` into six output bytes at `param_2[0..5]`. Pure format conversion — no hooks or side effects. Callers `FUN_80071138` (connection acceptance handler, HIGH-ready) and `accept_lmp_conn_setup_and_program_baseband_from_unpacked_pdu` (IRQ-masked connection-setup path that feeds unpacked bytes into indirect register callbacks). Sibling of Pass 12x's `lookup_esco_packet_type_table_entry_for_3_or_4_pair` in the `0x800718xx` LMP connection-setup cluster.
