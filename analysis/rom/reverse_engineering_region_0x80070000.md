...
**Next:** Pass 12em — decompile+rename `FUN_800658f0` (PSM/QoS state-0x16 even-retry sub-state callee, sibling of Pass 12el / Pass 12ek).
...
**Confidence:** HIGH — full decompilation; retry-0 sibling of Pass 12ek random walk and even-retry `FUN_800658f0` in Pass 12eg sub-state machine; uses same config threshold as cluster.
...
**Confidence:** HIGH — full decompilation; sibling of retry-0 `expand_psm_qos_state_0x16_retry0_sequential_channel_bitpair_eligibility` and even-retry `FUN_800658f0` in Pass 12eg sub-state machine; calls already-named `FUN_80042934` PRNG helper.
