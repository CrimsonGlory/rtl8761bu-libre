...
**Next:** superseded by Pass 205.

## Pass 205 (2026-07-01) — HW reg 0x23 bits 5-7 RMW `FUN_80039298`

Fresh `ListUnnamed80030000.java` re-run: **86 unnamed** remain in region
(unchanged from Pass 204; rank-1 by size at xref=1 tier is `FUN_80039298` at
52B — wins on address over tied 52B siblings).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80039298` → `read_modify_write_hw_reg_0x23_set_bits5_7_from_3bit_param`**
(52B, HIGH, SIMPLE-tier) via
`RenamePass205Region80030000Fun80039298.java` (`renamed=1`, live-verified).

**Mechanism:** Read-modify-write HW/VSC register `0x23` bits 5-7: reads via
indirect read fptr at `PTR_DAT_800392cc` with arg `0x23`, then writes back via
write fptr at `PTR_DAT_800392d0` with value
`(read_val & 0xff1f) | (param_1 & 0xe0)` — clears bits 5-7, replaces with
bits 5-7 of `param_1`. Sibling of Pass 204's
`read_modify_write_hw_reg_0x17_set_bits0_2_from_3bit_param` and Pass 199's
`read_modify_write_hw_reg_0x22_set_bits7_9_from_3bit_param` in the `0x800391xx`
BB register-config init cluster.

**Callers:** 1 xref-in — `FUN_800392d4` (config-bit6-gated optional hook: when
`PTR_DAT_800392f4[1]` bit 6 (`0x40`) set, passes config byte at offset `0x30`
to program bits 5-7 of reg `0x23`).

**Confidence:** HIGH — full 52B decompile; register index `0x23` and 3-bit field
merge at bits 5-7 unambiguous; caller config-gate pattern matches Pass 199's
`FUN_80039234` sibling (config bit `0x4`, offset `0x3d`).

Region unnamed count after this pass: **85** (86 minus this rename). Live named
**2081** global.

**Next:** superseded by Pass 206.

## Pass 206 (2026-07-01) — HW reg 0x18 bits 14-15 RMW `FUN_80038ee0`

Fresh `ListUnnamed80030000.java` re-run: **85 unnamed** remain in region
(unchanged from Pass 205; rank-1 by size at xref=1 tier is `FUN_80038ee0` at
52B — wins on address over tied 52B sibling `FUN_80033ce8`).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80038ee0` → `read_modify_write_hw_reg_0x18_set_bits14_15_from_2bit_param`**
(52B, HIGH, SIMPLE-tier) via
`RenamePass206Region80030000Fun80038ee0.java` (`renamed=1`, live-verified).

**Mechanism:** Read-modify-write HW/VSC register `0x18` bits 14-15: reads via
indirect read fptr at `PTR_DAT_80038f14` with arg `0x18`, then writes back via
write fptr at `PTR_DAT_80038f18` with value
`(read_val & 0x3fff) | (param_1 & 3) << 0xe` — clears bits 14-15, replaces with
bits 0-1 of `param_1`. Sibling of Pass 205's
...
`FUN_800392d4` sibling (config bit `0x40`, offset `0x30`).
...
**Next:** superseded by Pass 216.

## Pass 216 (2026-07-01) — register-script dual invoke `FUN_80039418`

Fresh `ListUnnamed80030000.java` re-run: **75 unnamed** remain in region
(unchanged from Pass 215; rank-1 by size at xref=1 tier is `FUN_80039418` at
38B).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80039418` → `invoke_register_script_dual_from_global_context_pairs`**
(38B, HIGH, SIMPLE-tier) via `RenamePass216Region80030000Fun80039418.java`
(`renamed=1`, live-verified).

**Mechanism:** Thin register-script interpreter dual-invoke wrapper in the
`0x800394xx` cluster. Loads global context `PTR_PTR_80039440`, calls through
hook fptr at `PTR_DAT_80039444` twice with embedded `(script_ptr,
num_halfwords)` pairs at context offsets `+0x00`/`+0x04` and `+0x08`/`+0x0c` —
matching `register_script_interpreter` signature. Sibling of
`invoke_register_script_from_global_context_0x60_0x64` (Pass 135) and
`FUN_800393f8` (offsets `+0x58`/`+0x5c`). Listed as caller of
`register_script_interpreter` (×2) in
`reverse_engineering_register_script_interpreter.md`.

**Callers:** 1 xref-in per `ListUnnamed80030000`; `xrefs_to` empty (indirect
dispatch).

**Confidence:** HIGH — full 38B decompile; dual hook invoke with two embedded
script pairs unambiguous; register-script cluster sibling of Pass 135.
...
**Next:** superseded by Pass 222.

## Pass 222 (2026-07-01) — config-bit1-gated HW reg 0x16 hook `FUN_80039358`

Fresh `ListUnnamed80030000.java` re-run: **69 unnamed** remain in region
(unchanged from Pass 221; rank-1 by size at xref=1 tier is `FUN_80039358` at
30B — wins on size over tied 30B siblings, first by address).

Decompiled and renamed rank-1 cold-triage target (caller side documented Pass
202 but remained `FUN_*` in Ghidra until this pass):
**`FUN_80039358` → `read_modify_write_hw_reg_0x16_bits1_3_if_config_byte1_bit2_set`**
(30B, HIGH, SIMPLE-tier) via
`RenamePass222Region80030000Fun80039358.java` (`renamed=1`, live-verified).

**Mechanism:** Thin config-bit1-gated optional hook in the `0x800393xx` BB
register-config init cluster: when config byte `PTR_DAT_80039378[1]` bit `0x2`
is set, calls `read_modify_write_hw_reg_0x16_set_bits1_3_from_3bit_param`
(Pass 202) with config byte at offset `0x3c`. Caller-side complement of Pass
202's reg `0x16` RMW writer; sibling of Pass 220's
`write_hw_reg_0x10_four_config_bytes_if_config_byte1_bit7_set` and Pass 204's
config-bit1-gated `FUN_80039334` (offset `0x39` byte).

**Callers:** 1 xref-in per `ListUnnamed80030000`; `xrefs_to` empty (indirect
dispatch).

**Confidence:** HIGH — full 30B decompile; config-byte bit `0x2` gate and
callee chain to Pass 202 unambiguous; closes caller-side documentation-vs-Ghidra
naming gap.

Region unnamed count after this pass: **68** (69 minus this rename). Live named
**2098** global.

**Next:** superseded by Pass 223.

## Pass 223 (2026-07-01) — config-bit0-gated HW reg 0x17 hook `FUN_80039334`

Fresh `ListUnnamed80030000.java` re-run: **68 unnamed** remain in region
(unchanged from Pass 222; rank-1 by size at xref=1 tier is `FUN_80039334` at
30B — wins on address over tied 30B siblings `FUN_80039234`/`FUN_80038f48`).

Decompiled and renamed rank-1 cold-triage target (caller side documented Pass
204 but remained `FUN_*` in Ghidra until this pass):
**`FUN_80039334` → `read_modify_write_hw_reg_0x17_bits0_2_if_config_byte0_bit2_set`**
(30B, HIGH, SIMPLE-tier) via
`RenamePass223Region80030000Fun80039334.java` (`renamed=1`, live-verified).

**Mechanism:** Thin config-bit0-gated optional hook in the `0x800393xx` BB
register-config init cluster: when config byte `PTR_DAT_80039354[0]` bit `0x2`
is set, calls `read_modify_write_hw_reg_0x17_set_bits0_2_from_3bit_param`
(Pass 204) with config byte at offset `0x39`. Caller-side complement of Pass
204's reg `0x17` RMW writer; sibling of Pass 222's
`read_modify_write_hw_reg_0x16_bits1_3_if_config_byte1_bit2_set` and Pass 220's
`write_hw_reg_0x10_four_config_bytes_if_config_byte1_bit7_set`.

**Callers:** 1 xref-in per `ListUnnamed80030000`; `xrefs_to` empty (indirect
dispatch).

**Confidence:** HIGH — full 30B decompile; config-byte bit `0x2` gate and
callee chain to Pass 204 unambiguous; closes caller-side documentation-vs-Ghidra
naming gap.
...
**Next:** superseded by Pass 230.

## Pass 230 (2026-07-01) — Register-script invoke wrapper `FUN_80039174`

Fresh `ListUnnamed80030000.java` re-run: **61 unnamed** remain in region
(unchanged from Pass 229; rank-1 at xref=1 tier is `FUN_80039174` at 22B —
largest among tied 22B xref=1 cohort).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80039174` → `invoke_register_script_from_global_context_0x38_0x3c`**
(22B, HIGH, SIMPLE-tier) via
`RenamePass230Region80030000Fun80039174.java` (`renamed=1`, live-verified).

**Mechanism:** Thin register-script interpreter invocation wrapper in the
`0x800391xx` cluster. Loads global context `PTR_PTR_8003918c`, calls through
hook fptr at `PTR_DAT_80039190` with `(script_ptr, num_halfwords)` =
`(*(ctx+0x38), *(ushort*)(ctx+0x3c))` — matching
`register_script_interpreter` signature. Sibling of
`invoke_register_script_from_global_context_0x58_0x5c` (Pass 229, offsets
`+0x58`/`+0x5c`), `invoke_register_script_from_global_context_0x60_0x64`
(Pass 135), and `invoke_register_script_dual_from_global_context_pairs`
(Pass 216). Caller of `register_script_interpreter` per
`reverse_engineering_register_script_interpreter.md`.

**Callers:** 1 xref-in per `ListUnnamed80030000`; `xrefs_to` empty (indirect
dispatch).

**Confidence:** HIGH — full 22B decompile; offset-pair invoke pattern
unambiguous; naming mirrors Pass 135/229 siblings.
...
**Next:** superseded by Pass 249.

## Pass 249 (2026-07-01) — parallel slot table active-state init `FUN_8003d068`

Fresh `ListUnnamed80030000.java` re-run: **42 unnamed** remain in region
(unchanged from Pass 248; xref_in=0 tier dominates — rank-1 is `FUN_8003d068`
at 100B, largest among xref=0 cohort).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_8003d068` → `init_parallel_slot_table_entry_active_state_from_dual_index`**
(100B, HIGH) via
`RenamePass249Region80030000Fun8003d068.java` (`renamed=1`, live-verified).

**Mechanism:** Parallel-slot-table entry initializer in the `0x8003d0`
cluster (`PTR_DAT_8003d0cc`, 0x88-byte stride). `param_2` encodes dual
indices: full low-16-bit index selects the primary slot (`+0x80..+0x82`
active flags, `+0x78` cleared, `+0x7a` bit1 cleared, `+0x70`/`+0x74` counter
handling); low-byte index selects a sub-slot (`+0x79` stores param low byte,
`+0x7a` bit0 cleared, copies template dword from `+0x30` to `+0x7c`). Setup
counterpart to Pass 68 `reset_parallel_slot_table_entry_tail_state_by_index`
and Pass 88 `reset_slot_tail_and_hook_dispatch_status_upper_bits_if_idle`.

**Callers:** 0 xref-in (consistent with indirect fptr-table invocation).

**Confidence:** HIGH — full 100B decompile; 0x88-stride field offsets and
dual-index encoding match documented parallel-slot-table cluster.
...
**Next:** superseded by Pass 272.

## Pass 272 (2026-07-01) — gather LUT reader `FUN_80033e6c`

Fresh `ListUnnamed80030000.java` re-run: **19 unnamed** remain in region
(unchanged from Pass 271; xref_in=0 tier dominates — rank-1 is `FUN_80033e6c`
at 36B, gather inverse of Pass 271's scatter sibling).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80033e6c` → `gather_9_uint32_from_lut_indexed_base_to_buffer`**
(36B, HIGH, UTILITY-tier) via
`RenamePass272Region80030000Fun80033e6c.java` (`renamed=1`, live-verified).

**Mechanism:** Gather-read loop (9 iterations): for each index `i`, reads
`*(uint32*)(DAT_80033e94 + *(ushort*)(PTR_DAT_80033e90 + i*2))` into
`*(uint32*)(param_1 + i*4)`. Scatter-write inverse is Pass 271's
`scatter_9_uint32_from_buffer_to_lut_indexed_base`. Completes the `0x80033e`
indexed-LUT gather/scatter pair.

**Callers:** 0 xref-in (consistent with indirect fptr-table invocation).

**Confidence:** HIGH — full 36B decompile; 9-iteration gather via ushort LUT
+ base unambiguous; scatter inverse at `scatter_9_uint32_from_buffer_to_lut_indexed_base`
confirms semantics.
...
**Next:** superseded by Pass 282.

## Pass 282 (2026-07-01) — inquiry-LAP slot selector wrapper `FUN_80034cac`

Fresh `ListUnnamed80030000.java` re-run: **9 unnamed** remain in region
(unchanged from Pass 281; xref_in=0 tier dominates — rank-1 is `FUN_80034cac`
at 16B, the tied sibling deferred from Pass 281).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_80034cac` → `invoke_find_first_inquiry_lap_slot_with_pending_clear_2`**
(16B, HIGH, UTILITY-tier) via
`RenamePass282Region80030000Fun80034cac.java` (`renamed=1`, live-verified).

**Mechanism:** Identical thin void wrapper to Pass 281's
`invoke_find_first_inquiry_lap_slot_with_pending_clear`: tail-calls
`find_first_inquiry_lap_slot_with_pending_clear()` (region `0x80040000` Pass 52z)
and discards the returned slot index. Adjacent at `0x80034cac` (16B before
`0x80034cbc`); likely duplicate fptr-table entry in the `0x80034cxx` packet-type /
conn-setup cluster.

**Callers:** 0 xref-in (consistent with indirect fptr-table invocation).

**Confidence:** HIGH — trivial 16B decompile; byte-identical semantics to Pass 281
sibling; callee already documented at `0x80042f2c`.

Region unnamed count after this pass: **8** (9 minus this rename). Live named
**2158** global.

**Next:** superseded by Pass 283.

## Pass 283 (2026-07-01) — global dword AND-mask `FUN_8003b758`

Fresh `ListUnnamed80030000.java` re-run: **8 unnamed** remain in region
(unchanged from Pass 282; xref_in=0 tier dominates — rank-1 is `FUN_8003b758`
at 12B, tied with `FUN_8003b744` at 12B; first-listed wins).

Decompiled and renamed rank-1 cold-triage target:
**`FUN_8003b758` → `and_mask_into_global_dword_at_indirect_ptr`**
(12B, HIGH, UTILITY-tier) via
`RenamePass283Region80030000Fun8003b758.java` (`renamed=1`, live-verified).

**Mechanism:** Trivial global-dword bit-clear via AND mask: loads pointer from
`DAT_8003b764`, mask from `DAT_8003b768`, executes `*ptr &= mask`. Lives in
the `0x8003b7xx` per-connection HW-buffer / BB-reg hook cluster adjacent to
Pass 197's `read_modify_write_hw_reg_0x44_set_bit1` (uses fptrs at
`PTR_DAT_8003b73c`/`PTR_DAT_8003b740` in same literal-pool neighborhood) and
Pass 152's `program_bb_regs_0x220_0x222_0x224_via_hook_with_masked_params`.
Tied sibling `FUN_8003b744` at `0x8003b744` is the complementary OR-mask
primitive (decompiled this pass, not renamed).

**Callers:** 0 xref-in (consistent with indirect fptr-table invocation).

**Confidence:** HIGH — trivial 12B decompile; AND-mask semantics unambiguous;
