...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **759** as of region 0x80050000 PASS 11-batch-2 + PASS 12 (2026-06-27: 16 newly-confirmed-named — 6 already applied in an undocumented prior session, 10 freshly applied via `RenameEscoNegotiationCluster.java`, `renamed=10 alreadyOk=0 missing=0 failed=0`) on top of the 743 from PASS 10's real execution, which itself sat on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2253** (2269 prior to this pass's 16 new renames) |
| Named-function rows in this doc's table (fresh grep, 2026-06-26; +6 PASS 10 2026-06-27; +16 PASS 11-batch-2/PASS 12 2026-06-27) | 804 rows / **774 unique addresses** (30 addresses still have 2 rows each — pre-existing duplicate-row debt, not newly introduced; see "Final reconciliation" below) |
| Doc-table confidence: **high** (decompiled + written up) | 803 rows (includes 10 rows upgraded from a mislabeled `medium` and 9 rows folded from an informal `medium-high` bucket, plus 6 PASS 10 (2026-06-27) rows and 16 PASS 11-batch-2/PASS 12 (2026-06-27) rows — see below) |
