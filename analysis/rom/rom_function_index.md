...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **767** as of region 0x80050000 PASS 20 (2026-06-27: 4 newly-confirmed-named — `le_channel_selection_algorithm_periodic_timing_check`/`binary_search_sorted_table_by_8byte_key`/`conn_slot_alloc_type01_and_store_bdaddr`/`conn_slot_alloc_type2_and_store_bdaddr_and_keys` via `RenamePass20Region80050000.java`, `RENAMED` ×4 — breaks the 4-consecutive-0-HIGH streak from Passes 16-19) on top of the 763 from PASS 15 (Passes 16-19 yielded 0 new HIGH renames), which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2245** (2249 prior to this pass's 4 new renames) |
