...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **854** as of region 0x80050000 Pass 26 (2026-06-27: 6 newly-renamed — `atomic_increment_dword_at_ptr_plus0x28`/`send_LMP_25B_if_both_flags_clear`/`write_hw_reg_bits15_14_A`/`write_hw_reg_bits15_14_B`/`write_connection_struct_fields_1c_1e_20_to_hw_regs`/`LE_connection_channel_update_timing_handler` via `RenamePass26Region80050000.java`, `RENAMED` ×6) on top of the 848 from region 0x80050000 Pass 25 (4 renames), on the 844 from Pass 24 (10 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
...
| `0x80050000`–`0x8005ffff` | 366 | 88 | 278 | 19.7% |
