...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **790** as of region 0x80050000 PASS 23 (2026-06-27: 12 newly-confirmed-named — `dispatch_afhca_by_role_index`/`write_link_register_with_slot_check_and_retry`/`write_link_register_b_with_slot_check_and_retry`/`write_indexed_link_register_b_with_slot_check`/`write_link_type_hw_register_cmd`/`alloc_tag11_record_with_params`/`alloc_tagd_record_with_param`/`dispatch_alloc_tag_d_or_11_by_record_flag`/`alloc_tag_record_copy_payload_and_enqueue`/`write_sco_esco_link_band_regs_with_retry`/`write_sco_link_slot_params_type_b`/`write_sco_link_slot_params_type_c` via `RenamePass23Region80050000.java`, `RENAMED` ×12) on top of the 778 from PASS 22 (8 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
