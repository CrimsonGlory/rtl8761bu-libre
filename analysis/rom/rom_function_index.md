...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **848** as of region 0x80050000 Pass 25 (2026-06-27: 4 newly-renamed — `write_sco_link_slot_params_type_d`/`check_pending_features_and_alloc_mismatch_record`/`enqueue_to_per_slot_ring_buffer`/`read_link_register_0xe_top_nibble_by_slot` via `RenamePass25Region80050000.java`, `RENAMED` ×4) on top of the 844 from region 0x80050000 Pass 24 (10 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
...
| `0x80050000`–`0x8005ffff` | 366 | 82 | 284 | 18.3% |
