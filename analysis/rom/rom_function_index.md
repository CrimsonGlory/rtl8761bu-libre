...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **831** as of region 0x80000000 out-of-gap-scope pass 2 (2026-06-27: 25 newly-renamed — `spin_delay_10x_iterations`/`spin_delay_10000x_iterations`/`read_byte_counter_1`/`read_byte_counter_2`/`byte_add_to_counter_no_irq`/`atomic_byte_write_zero_cnt1`/`set_channel_bdaddr_scramble_fields`/`atomic_saturating_byte_decrement_by1_cnt1`/`atomic_byte_add_to_cnt`/`atomic_byte_write_zero_cnt2`/`atomic_saturating_byte_decrement_by1_cnt2`/`atomic_byte_increment_cnt`/`stub_return_status_ok`/`call_fptr_if_set_wraps_check_slot_state_eq2`/`wrap_struct_field_accessor_0x80100000`/`write_hw_register_bit8_validated`/`configure_hw_register_fields_by_mode`/`set_hw_feature_bit8_and_companion_flag`/`drain_hci_cmd_queue_12slots_code3`/`drain_hci_cmd_queue_6slots_code2`/`drain_hci_cmd_queue_8slots_code1`/`drain_hci_cmd_queue_32slots_return1`/`pool_subsystem_mode_switch_with_drain`/`stub_return_zero`/`check_slot_state_is_2` via `RenamePass2Region80000000.java`, `RENAMED` ×25) on top of the 806 from out-of-gap-scope pass 1 (16 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
