...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **770** as of region 0x80050000 PASS 21 (2026-06-27: 3 newly-confirmed-named — `alloc_tagged_record_via_pool`/`release_kind_sized_subrecord`/`alloc_kind_record_and_clear_tail` via `RenamePass21Region80050000.java`, `RENAMED` ×3) on top of the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2242** (2245 prior to this pass's 3 new renames) |
