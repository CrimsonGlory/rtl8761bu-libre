...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **834** as of region 0x80000000 out-of-gap-scope pass 3 (2026-06-27: 3 newly-renamed — `disable_interrupts_return_status_v2`/`dispatch_hci_cmd_completion_from_queue_n_times`/`read_hw_register_bit8` via `RenamePass3Region80000000.java`, `RENAMED` ×3) on top of the 831 from out-of-gap-scope pass 2 (25 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
