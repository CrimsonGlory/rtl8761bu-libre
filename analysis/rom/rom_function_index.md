...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **763** as of region 0x80050000 PASS 15 (2026-06-27: 2 newly-confirmed-named — `clear_pending_procedure_bit_and_finalize_if_idle`/`commit_link_power_state_bits_to_hw_register_with_retry` via `RenamePass15Region80050000.java`, `RENAMED` ×2) on top of the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2249** (2251 prior to this pass's 2 new renames) |
