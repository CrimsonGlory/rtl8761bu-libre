...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **844** as of region 0x80050000 Pass 24 (2026-06-27: 10 newly-renamed — `remove_entry_from_sorted_8byte_key_table`/`send_LMP_268_with_slot_timing_adjustment`/`send_LMP_25B_and_reset_crypto_key_state`/`conditional_dispatch_LE_channel_selection_algorithm`/`write_hw_channel_reg_pair_a`/`write_hw_channel_reg_pair_b`/`write_hw_channel_reg_pair_c`/`commit_pending_proc_bit0x1000_if_cap_not_set`/`dispatch_and_commit_pending_proc_bit0x10_reason5`/`complete_LE_DLE_pending_proc_and_send_data_length_change_evt` via `RenamePass24Region80050000.java`, `RENAMED` ×10) on top of the 834 from region 0x80000000 out-of-gap-scope pass 3 (3 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
...
| `0x80050000`–`0x8005ffff` | 366 | 78 | 288 | 18.3% |
