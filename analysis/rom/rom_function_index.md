...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **761** as of region 0x80050000 PASS 14 (2026-06-27: 2 newly-confirmed-named — `find_duplicate_bdaddr_and_disconnect`/`esco_sco_param_validate_and_commit` via `RenamePass14Region80050000.java`, `renamed=2 alreadyOk=0 missing=0 failed=0`) on top of the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2251** (2253 prior to this pass's 2 new renames) |
