...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **743** as of region 0x80050000 PASS 10's real execution (2026-06-27: 6 fresh renames — `RenameConnTypeDispatchCluster.java`, `renamed=6 alreadyOk=0 missing=0 failed=0`, independently re-verified via `decompile_function`) on top of the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
| Unnamed (`FUN_*`) functions in `rom` block (live Ghidra ground truth) | **2269** (2275 prior to PASS 10's 6 new renames) |
| Named-function rows in this doc's table (fresh grep, 2026-06-26; +6 PASS 10 2026-06-27) | 788 rows / **758 unique addresses** (30 addresses still have 2 rows each — pre-existing duplicate-row debt, not newly introduced; see "Final reconciliation" below) |
| Doc-table confidence: **high** (decompiled + written up) | 787 rows (includes 10 rows upgraded from a mislabeled `medium` and 9 rows folded from an informal `medium-high` bucket, plus 6 new PASS 10 (2026-06-27) rows — see below) |
