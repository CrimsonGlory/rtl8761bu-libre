...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **861** as of region 0x80050000 Pass 27 (2026-06-27: 7 newly-renamed — `commit_esco_parent_timing_to_slot_table`/`validate_and_dispatch_connection_command_by_type`/`program_hw_channel_and_slot_params`/`compute_and_store_esco_phase_offset_from_link_regs`/`compute_esco_negotiated_packet_type_codes`/`check_esco_timing_window_and_trigger`/`compute_and_store_link_timing_in_slots` via `RenamePass27Region80050000.java`, `RENAMED` ×7) on top of the 854 from Pass 26 (6 renames), 848 from Pass 25 (4 renames), 844 from Pass 24 (10 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
...
| `0x80050000`–`0x8005ffff` | 366 | 95 | 271 | 25.9% |
