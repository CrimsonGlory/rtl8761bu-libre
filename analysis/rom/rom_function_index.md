...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **778** as of region 0x80050000 PASS 22 (2026-06-27: 8 newly-confirmed-named — `pending_procedure_advance_and_commit`/`initiate_pending_procedure_or_defer`/`alloc_tag18_record_and_snapshot_timing`/`alloc_tag1_record_and_snapshot_timing`/`alloc_tag0_record_and_snapshot_timing`/`alloc_tag3_record_and_copy_link_fields`/`read_indexed_link_register`/`write_indexed_link_register_with_slot_check` via `RenamePass22Region80050000.java`, `RENAMED` ×8) on top of the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
