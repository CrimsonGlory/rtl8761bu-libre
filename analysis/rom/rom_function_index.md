...
| User-named functions in `rom` block (live Ghidra, `SourceType.USER_DEFINED`/`IMPORTED` — ground truth) | **806** as of region 0x80000000 out-of-gap-scope pass 1 (2026-06-27: 16 newly-renamed — `exception_handler_save_regs_and_dispatch`/`set_status_reg_clear_exl_set_ie`/`set_status_reg_clear_exl_enable_im3`/`enable_all_interrupts_clear_exl`/`disable_interrupts_return_status`/`count_leading_zeros_32`/`atomic_saturating_byte_decrement`/`init_baseband_hw_from_config_struct`/`call_fptr_if_set_wraps_packet_send_5args`/`call_fptr_if_set_wraps_packet_slot_flush_check`/`call_fptr_if_set_wraps_pool_slot_reset`/`call_fptr_if_set_wraps_pool_slot_init_and_zero`/`drain_all_hci_cmd_completion_slots_once`/`send_packet_via_mmio_slot_table`/`flush_check_packet_slot`/`clear_packet_slot_descriptor_fields` via `RenamePass1Region80000000.java`, `RENAMED` ×16) on top of the 790 from region 0x80050000 PASS 23 (12 renames), which sat on the 770 from PASS 21 (3 renames), which sat on the 767 from PASS 20 (4 renames, broke the Passes 16-19 streak), which sat on the 763 from PASS 15, which sat on the 761 from PASS 14, which sat on the 759 from PASS 11-batch-2 + PASS 12, which itself sat on the 743 from PASS 10's real execution, on the 737 from the 2026-06-27 rename re-application pass (was 462 at the 2026-06-26 baseline) |
