...
**Next:** superseded by Pass 6 continuation (238).

## Pass 6 continuation (238) (2026-07-01) — VSC config bit-3 setter `FUN_8002fa64`

Decompiled and renamed:
**`FUN_8002fa64` → `set_config_byte_bit3_from_enable_byte`**
(44B, HIGH) via `RenamePass6Region80020000Fun8002fa64.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 by size among remaining unnamed (44B, xref_in=0) per fresh
`ListUnnamed80020000.java` run (`total_unnamed=75` at pass start). First-listed in tied
44B tier ahead of `FUN_8002fa34` (bit 2) and `FUN_80024400`; sibling deferred from Pass 6
cont. (186)/(202) bit-setter cluster documentation.

**Mechanism:** VSC config flag setter: when `param_2==1`, mirrors enable byte `*param_1`
into bit 3 of config struct byte `PTR_PTR_8002fa90+8` (`puVar1[8] = puVar1[8] & 0xf7 |
(*param_1 != '\0') << 3`). Returns `0x12` on bad params, `0` on success. Same idiom as
siblings `set_config_byte_bit4_and_status_word_bit15_from_enable_byte` (bit 4),
`set_config_byte_bits0_and_1_from_enable_pair` (bits 0–1), and pending `FUN_8002fa34`
(bit 2).
...
`0x8002f9xx` VSC config cluster; naming pattern consistent with Pass 6 cont. (186)/(202).
...
**Next:** superseded by Pass 6 continuation (241).

## Pass 6 continuation (241) (2026-07-01) — LC TX subcase-2 relay `FUN_8002ef18`

Decompiled and renamed:
**`FUN_8002ef18` → `relay_lc_tx_subcase2_hci_evt_0x453_and_completed_packets`**
(42B, HIGH) via `RenamePass6Region80020000Fun8002ef18.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 by size among remaining unnamed (42B, xref_in=2) per fresh
`ListUnnamed80020000.java` run (`total_unnamed=72` at pass start). First-listed in tied
42B tier; deferred structural twin of Pass 6 cont. (236)'s subcase-3 relay
`relay_lc_tx_subcase3_hci_evt_0x454_and_completed_packets`.

**Mechanism:** LC TX logger-hook subcase-2 relay when global mode byte `field_0x179==1`.
Three-step thin wrapper: (1) `clone_hci_evt_buffer_and_dispatch_hci_evt_0x453_logger_hook`
(tag **0x453**), (2) `send_evt_HCI_Number_Of_Completed_Packets` with connection handle
from `*param_1 & 0xfff` (low 12 bits — differs from subcase-3's `>> 8 & 0xfff`), (3)
secondary hook at `PTR_DAT_8002ef44` invoked as `(param_1, 2)`.

**Callers:** sole direct caller `dispatch_lc_tx_logger_hook_subcases_with_pending_queue`
at `0x8002ef48` (subcase 2 arm when `field_0x179==1`; documented Pass 6 cont. 36).

**Confidence:** HIGH — established LC TX subcase relay idiom with named callees and
documented parent dispatcher; structural twin of Pass 6 cont. (236)'s subcase-3 path and
Pass 6 cont. (81)'s `0x453` buffer-clone callee.
...
**Next:** superseded by Pass 6 continuation (248).

## Pass 6 continuation (248) (2026-07-01) — HCI passkey-reply thin wrapper `FUN_8002384c`

Decompiled and renamed:
**`FUN_8002384c` → `hci_resolve_conn_fhci_user_passkey_request_reply_0x34`**
(40B, HIGH) via `RenamePass6Region80020000Fun8002384c.java` (`renamed=1`, live-verified).

**Triage note:** Rank-1 by size among remaining unnamed (40B, xref_in=1) per fresh
`ListUnnamed80020000.java` run (`total_unnamed=65` at pass start). First-listed in tied
40B tier; sits in the `0x800238xx` SSP HCI-reply cluster alongside
`fHCI_User_Passkey_Request_Reply_0x34` (Pass 6 cont. 99) and sibling thin wrappers
`hci_resolve_conn_tail_call_dispatch_ssp_lmp_ext_0x1b_or_dhkey_0x41` (Pass 6 cont. 247)
and `hci_resolve_conn_ssp_pairing_continuation_lmp_ext_0x19_auth_failure` (Pass 6 cont. 226).

**Mechanism:** Thin HCI command wrapper — `hci_resolve_conn_record_validate_and_complete`
with validator `PTR_LAB_800235cc_1`, then on success calls
`fHCI_User_Passkey_Request_Reply_0x34(conn_handle)` and returns its result; returns 0
on conn-resolve failure. Same conn-resolve→fHCI-handler idiom as Pass 6 cont. (247)'s
`hci_resolve_conn_tail_call_dispatch_ssp_lmp_ext_0x1b_or_dhkey_0x41` but targets the
HCI User Passkey Request Reply (opcode 0x0434 / OCF 0x34) path documented in Pass 6 cont. (99).

**Callers:** `HCI_Write_Simple_Pairing_Debug_Mode` at `0x8002350c`; xref_in=1 per
`ListXrefsTo8002384c.java`.

**Confidence:** HIGH — decompile confirms standard HCI conn-resolve preamble plus
call to documented SSP passkey-entry reply handler; caller confirmed in SSP debug-mode
command router.
...
**Next:** superseded by Pass 6 continuation (304).

## Pass 6 continuation (304) (2026-07-01) — crypto global dword-pair writer `FUN_80021f08`

Decompiled and renamed:
**`FUN_80021f08` → `store_dword_pair_to_crypto_global_offsets_a8_ac`**
(14B, HIGH) via `RenamePass6Region80020000Fun80021f08.java` (`renamed=1`, live-verified).

**Triage note:** Skipped rank-1 `FUN_80024004` (24B, xref_in=0) and `FUN_8002963c` (22B,
xref_in=0) per established cold-triage convention; selected rank-1 with highest xref_in≥1:
`FUN_80021f08` (14B, xref_in=1) per fresh `ListUnnamed80020000.java` run
(`total_unnamed=9` at pass start).

**Mechanism:** Second sibling in the BT-stack cold-init crypto global dword-pair writer
cluster. Stores `param_1` and `param_2` to `PTR_DAT_80021f18+0xa8` and `+0xac`, returns 1.
Complements Pass 6 cont. (302)'s `store_dword_pair_to_crypto_global_offsets_a0_a4` (`+0xa0/+0xa4`)
and Pass 6 cont. (303)'s `store_dword_pair_to_crypto_global_offsets_b0_b4` (`+0xb0/+0xb4`).
Called from `FUN_800681d8` (region `0x80060000`) during cold-init alongside the other pair
writers — completes the three-offset dword-pair seeding sequence.

**Callers:** `FUN_800681d8` (xref_in=1) — BT stack cold-init orchestrator (same caller as
siblings at `+0xa0/+0xa4` and `+0xb0/+0xb4`).

**Confidence:** HIGH — decompile confirms unambiguous dual-dword store; single caller in
documented cold-init chain; completes the three-function crypto-global offset writer cluster.
