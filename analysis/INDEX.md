...
| `rom/reverse_engineering_region_0x80000000.md` | Phase 9 exhaustive-sweep catch-all for ROM region `0x80000000`-`0x8000ffff`'s two unexplored gaps (`~0x80000200`-`0x80009000`, `0x8000a474`-`0x8000ffff`; boot preamble/interrupt trampoline/USB transport sub-ranges excluded, already covered elsewhere). Resolved 12 functions to real names this pass: CP0 `Cause`/`EPC` register accessors, an ISR bottom-half status dispatcher, USB/connection/timer/UART event-status handlers, a baseband 13-source event dispatcher, a conn-record linked-list drain primitive, and a mark-dirty/clear-dirty pair extending `conn_record_subsystem.md`. Verifies (per CLAUDE.md's ask) that Ghidra function renames persist across separate `run_ghidra_headless` calls against the same `use_saved_project=True` GZF cache — confirmed yes. ~25 more functions decompiled with partial evidence but not yet confidently named (listed for the continuation ticket); ~208 of 220 gap functions remain untriaged |
